...
v0.3.0 — WS transport + CLI + MCP server。`pkg/channel/websocket`
（gorilla/websocket，`BinarySafe=true`）可直接对接 `socat ws ↔ bash`
风格的桥，并支持通过 `Encode`/`Decode` hook 适配 ttyd / wetty /
code-local。`cmd/ptyrelay` 是命令行入口（exec/get/put/stat/ls/
bootstrap/agent-info），`cmd/ptyrelay-mcp` 把 RouterBackend 包成
MCP tool provider，Claude Code 可直接 discover。Agent REPL 比一次性
模式快 ~283×（见 `pkg/backend/agent/bench_test.go`）。详见
[TODOs.md](TODOs.md)、[docs/TRANSPORTS.md](docs/TRANSPORTS.md)、
