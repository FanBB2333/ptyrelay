...
v0.3.0 在路上：通用 `pkg/channel/websocket`（gorilla/websocket，
`BinarySafe=true`）已落地——可直接对接 `socat ws ↔ bash` 风格的桥，
也支持通过 `Encode`/`Decode` hook 适配 ttyd / wetty / code-local
等带前缀字节的协议。详见 [TODOs.md](TODOs.md)、
